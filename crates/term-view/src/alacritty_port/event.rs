...
impl Processor {
    ...
    pub fn new(
        ...
        let proxy = EventSender::from_winit_proxy(event_loop.create_proxy());
...
impl EventSender {
    pub fn new(send: impl Fn(Event) + Send + Sync + 'static) -> Self {
        Self { send: Arc::new(send) }
    }

    pub fn from_winit_proxy(proxy: EventLoopProxy<Event>) -> Self {
        Self::new(move |event| {
...
impl<'a, N: Notify + 'a, T: EventListener> input::ActionContext<T> for ActionContext<'a, N, T> {
    ...
    fn create_new_window(&mut self, #[cfg(target_os = "macos")] tabbing_id: Option<String>) {
        ...
        self.event_proxy.send(Event::new(EventType::CreateWindow(options), None));
    }

    #[cfg(windows)]
    fn create_new_window(&mut self) {
        self.event_proxy
            .send(Event::new(EventType::CreateWindow(WindowOptions::default()), None));
    }
    ...
    pub fn handle_event(&mut self, event: WinitEvent<Event>) {
        match event {
            WinitEvent::UserEvent(Event { payload, .. }) => match payload {
                ...
                | EventType::CreateExternalWindow
                | EventType::Frame => (),
            },
...
impl EventProxy {
    pub fn new(proxy: EventSender, window_id: WindowId) -> Self {
        Self { proxy, window_id }
    }

    /// Send an event to the event loop.
    pub fn send_event(&self, event: EventType) {
        self.proxy.send(Event::new(event, self.window_id));
    }
}

impl EventListener for EventProxy {
    fn send_event(&self, event: TerminalEvent) {
        self.proxy.send(Event::new(event.into(), self.window_id));
    }
}
