...
fn alacritty(mut options: Options) -> Result<(), Box<dyn Error>> {
    ...
    let event_sender = EventSender::from_winit_proxy(window_event_loop.create_proxy());

    // Initialize the logger as soon as possible as to capture output from other subsystems.
    let log_file = logging::initialize(&options, event_sender.clone())
    ...
    let socket_path = if config.ipc_socket() {
        match ipc::spawn_ipc_socket(&options, event_sender.clone()) {
            Ok(path) => Some(path),
